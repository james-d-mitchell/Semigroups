...
# Returns the normal closure of the inverse subsemigroup generated by <list>

BindGlobal("_UnbindOrbSCC", function(o)
  Unbind(o!.scc);
  Unbind(o!.trees);
  Unbind(o!.scc_lookup);
  Unbind(o!.mults);
...
BindGlobal("_AddGeneratorsNormalClosure", 
function(o, x) 
end);

DeclareRepresentation("IsNormalInverseSubsemigroupRep", 
                      IsComponentObjectRep 
                      and IsAttributeStoringRep
                      and IsInverseActingSemigroupRep, 
                      []);

NormalSubsemigroup := function(S, list)
  local filts, N;
  filts := IsNormalInverseSubsemigroupRep;
  N := Objectify(NewType(FamilyObj(list), filts), rec());
  SetParent(N, S);
  SetRepresentative(N, list[1]);
  return N;
end;

LambdaOrbSchutzNormalSgp := function(o, m, l)
  local S, gens, scc, lookup, orbitgraph, genstoapply, lambdaperm, rep, rank,
  one, bound, G, stop, forward, f, k;
...
"for an inverse acting semigroup and homog. list", 
[IsInverseActingSemigroupRep, IsHomogeneousList],
function(S, list)
  local N, o, orb, op, ht, orbitgraph, nr, gens, htvalue, K, k, scc, i, m;

  # if not IsSubsemigroup(S, N) then
  #   Error("the 2nd argument (an inverse semigroup) is not a subsemigroup ", 
  #         "of the 1st argument (an inverse semigroup)");
  # elif not IsFullInverseSubsemigroup(S, N) then
  #   Error("the 2nd argument (an inverse semigroup) is not a full ",
  #         "subsemigroup of the 1st argument (an inverse semigroup)");
  # fi;

  N := Semigroup(list);
  if IsSemilattice(N) then
    return N;
  fi;

  list := Filtered(list, x -> not IsIdempotent(x));
  o := StructuralCopy(Enumerate(LambdaOrb(S)));
  _UnbindOrbSCC(o);
  Unbind(o!.schutz);
  Unbind(o!.schutzstab);
  # Maybe unbind schreiergen + schreierpos too?

  o!.parent := N;
  o!.scc_reps := [FakeOne(GeneratorsOfSemigroup(S))];
  o!.orbitgraph := List([1 .. Length(o)], x -> []);
  ...
  while k <= K do
    Add(gens, list[k]);
    for i in [1 .. nr] do
      orbitgraph[i][k]     := htvalue(ht, op(orb[i], list[k]));
      orbitgraph[i][K + k] := htvalue(ht, op(orb[i], list[k] ^ -1));
      # Recompute the SCCs
      _UnbindOrbSCC(o);
      scc := OrbSCC(o);
      # Recompute the normal closures of the Schutzenberger groups
      for m in [1 .. Length(scc)] do
        LambdaOrbSchutzNormalSgp(o, m, k);
      od;
    od;
    k := k + 1;
  od;
